...
redistributors.** The eighteen items under *Breaking* change behaviour you may
...
- **`color(v, min, max)` is gone, replaced by `ramp`.** There is **no alias**: a program calling `color` fails on that line. `ramp.hues(v, min, max)` is what it was — a number mapped onto the colour wheel — and there is now one ramp per block category beside it: `ramp.colors`, `ramp.glass`, `ramp.lamps`, and `ramp.<name>` for a category a game has registered. Only `ramp.hues` reads as a smooth gradient; the others walk light, plain and dark inside each family in turn, so a gradient across one of them strobes. Values outside `[min, max]` clamp instead of wrapping past the palette, as `color` did from this release
