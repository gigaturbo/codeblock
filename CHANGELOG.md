...
redistributors.** The nineteen items under *Breaking* change behaviour you may
...
- **Three unused helpers are gone from `codeblock.utils`**: `table_reverse`, `table_convert_ik` and `table_convert_iv`. Nothing in the mod ever called them, but `codeblock.utils` is a global this mod publishes, so a game or another mod reading one of the three now reads `nil`. They were four lines each — copy the one you need into your own mod
