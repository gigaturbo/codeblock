...
redistributors.** The thirteen items under *Breaking* change behaviour you may be
relying on.
...
- `/codegenerate` no longer overwrites existing files
...
- `/codelevel` being unusable in singleplayer, and `/codegenerate` having no privilege check and ignoring its playername
...
- The check that every codelevel limit has a documented row, which matched by name and so never checked `pace_ms`, `heap_mb` or `map_memory_mb`
