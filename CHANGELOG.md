...
- [x] Added a **live view of what a running program is spending**, in two places. A small display in the top-right corner while your program runs names the file, says whether it is running or paused, and shows the single limit the run is closest to as a percentage, coloured as it fills - so you learn which resource your programs actually spend rather than being told only once they stop. And **left clicking a drone with the drone setter** opens a panel: all four limits with what the run has spent beside each ceiling, in the same units `minetest.conf` uses, plus **Pause**, **Resume**, **Cancel** and **Remove drone**. **Note, this changes an existing gesture:** that click used to end the run outright with nothing asked, and removing a drone is now the button rather than the click - one step further away, deliberately, because there was no way to undo an accidental one. A paused program holds its place indefinitely, is charged no running time while it waits, and gives its share of the server's step to the other drones. The corner display can be turned off per player with *Show the drone HUD* in the editor's *Settings* panel, or for everyone with `codeblock_drone_hud = false`; a player's own choice wins over the server's
- [x] The **drone panel and HUD** now say something a player can act on, after the first session with them found two ways they misled. **The corner display no longer names *map memory* almost every time.** That row is a throttle rather than a deadline — the drone waits when it is full and the engine frees the memory by itself, so it sits at 100% for any large build by design — and it was drowning out the three limits that actually stop a program. It is still shown, and now reads `throttled` instead of a percentage that looked like imminent failure. **And *Running time* was renamed *Server time used*, because it never was clock time:** a drone is charged only the time the server actually gave it, which is roughly a tenth of the time you watch pass, so `max_runtime_s` of 1800 at codelevel 4 is nearer five hours than thirty minutes. Nothing about what is counted has changed — only what it is called, and every row now carries a line saying what it is and whether reaching it stops the program. Long counts are readable too: `1.2K / 100.0M` rather than `1247 / 100000000`, with a percentage on every row instead of only the summary line
