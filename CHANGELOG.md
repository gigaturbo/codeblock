...
- [x] Added a **live view of what a running program is spending**, in two places. A block in the top-right corner while your program runs: the file and whether it is running or paused, then `Budget usage:` and one line each for **Blocks**, **CPU** and **Memory** as percentages - so you can see which resource your programs actually spend rather than being told only once they stop. The limit that will be reached first is **amber**, anything at 80% or more is **red**, and a run nowhere near a ceiling shows no colour at all. And **left clicking a drone with the drone setter** opens a panel: all four limits with what the run has spent beside each ceiling, in the same units `minetest.conf` uses, plus **Pause**, **Resume** and **Stop**. **Note, this changes an existing gesture:** that click used to end the run outright with nothing asked, and stopping a drone is now the button rather than the click - one step further away, deliberately, because there was no way to undo an accidental one. A paused program holds its place indefinitely, is charged no running time while it waits, and gives its share of the server's step to the other drones. The corner display can be turned off per player with *Show the drone HUD* in the editor's *Settings* panel, or for everyone with `codeblock_drone_hud = false`; a player's own choice wins over the server's
- [x] The **drone panel and HUD** now say something a player can act on, after the first session with them found two ways they misled. **Neither display mentions *map memory* any more.** That row is a throttle rather than a deadline — the drone waits when it is full and the engine frees the memory by itself, so it sits at 100% for any large build by design — and it was drowning out the three limits that actually stop a program. Both the panel and the corner display now list **only the limits that stop a program**, three of them. **And *Running time* was renamed *Server time used*, because it never was clock time:** a drone is charged only the time the server actually gave it, which is roughly a tenth of the time you watch pass, so `max_runtime_s` of 1800 at codelevel 4 is nearer five hours than thirty minutes. Nothing about what is counted has changed — only what it is called, and every row now carries a line saying what it is and whether reaching it stops the program. Long counts are readable too: `1.2K / 100.0M` rather than `1247 / 100000000`, with a percentage on every row and no summary line at all - instead, the limit that will be reached first has its percentage in **amber**, and anything at 80% or more goes **red**, so there is either nothing to look at or one thing. Each limit's name is bold with its explanation underneath, wrapped over two lines so a translation is not cut off
- [x] That panel opens **whatever the drone is doing** — including when you have no drone at all, where it says so rather than doing nothing. A gesture whose effect depends on state you cannot see is one you have to guess at, and the guess destroyed builds
- [x] The two tool icons were redrawn. Their editable sources are now SVG (`textures/*.svg`) rather than GIMP `.xcf`, and neither ships in the release archive - a player needs the PNG the engine loads and nothing else
- [x] The editor's three preference checkboxes — **Load program on exit**, **Save on tab switch** and **Show the drone HUD** — moved onto the **Settings** panel, beside the default-block picker, under a *Preferences* heading. They used to sit loose along the bottom edge of the form
