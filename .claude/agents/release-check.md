...
**Read `.claude/skills/generated-files/SKILL.md` before gates 4, 5 and 9.** It
holds `lib/api.lua` as the single source of the API, and the seven things here
that restate the source and drift in silence — `doc/api.md`,
`locale/template.txt`, `settingtypes.txt`, `.luacheckrc`'s sandbox std,
`.cdb.json`, the new-file template in `lib/formspecs.lua` and the example-name
list in `tests/preprocess_spec.lua`. Which of them a `--check` covers, and which
it does not, is exactly what those gates turn on.
...
- **`doc/api.md` is generated only from its `# Lua api` heading onward.** The
  region above it — `# Codelevel` and `# Chat commands` — is hand-written and
  `--check` never reads it. A green check has already stood over chat commands
  that no longer existed. Read that region against the registered commands
  yourself.
...
- **ContentDB** — `.cdb.json`, generated from `CONTENTDB.md` by
  `scripts/gen_cdb_json.sh`. **Nothing verifies it**, so compare
  `long_description` against the current `CONTENTDB.md` and say whether the
  generator has been re-run since the last edit. That it ran is not enough: see
  gate 9, which is about whether the *content* belongs there at all.
...
- **`.gitattributes`.** ContentDB builds the release with `git archive`, and **no
  ...
  `scripts/`, `screenshots/`, `doc/*.png`, `*.svg`, `*.xcf`, and the record —
  `ROADMAP.md`, `TODO.md`, `AUDIT.md`, `PLAYTEST.md`, `CLAUDE.md` and
  `CONTENTDB.md`, each named on its own line in `.gitattributes`. A new file at
  the root, or a new directory, is the thing that slips through. `screenshot.png`
  must survive as `-export-ignore` — Luanti shows it in the main menu's Mods tab.
  `doc/api.md` must survive too: the shipped README tells the player to read it.
...
**Known state, and do not re-derive it.** `C19` is **resolved**:
`long_description` is no longer `README.md`, which broke six of those rules at
once. Its source is `CONTENTDB.md`, whose sections are *Features*, *Quick
start*, *Important notes* and *Recent changes*. Check that source against the
list above, then check that `.cdb.json` was regenerated from it.

Three smaller things while you are in these files:

- **`CONTENTDB.md`'s *Recent changes* list is hand-kept against
  `CHANGELOG.md`** and nothing checks the two agree. Read them side by side.
- **Every ContentDB URL in `README.md` is on `content.minetest.net`**, the
  pre-rename domain. It redirects; it is still stale. `README.md` also still
  claims the mod *"works in any game that provides the blocks it places"*, which
  has been false and backwards since the mod started registering its own 105
  nodes.
- **`short_description`** should be specific about what the package contains,
  not a generic category line.
